...
Object.keys(corpses).forEach(corpse => {
    makeAnnotation(new RegExp(`You see here (?:an? (?:partly eaten )?)?(?:cursed |uncursed |blessed )?${corpse}(?:'s?)? corpse\\.`),
        `Corpse: ${corpses[corpse]}`)
    makeAnnotation(new RegExp(`This is (?:an? (?:partly eaten )?)?(?:cursed |uncursed |blessed )?${corpse}(?:'s?)? corpse here; eat it\\?`),
        `Corpse: ${corpses[corpse]}`)
    makeAnnotation(new RegExp(`It smells like ${corpse}\\.`),
        `Tin: ${corpses[corpse]}`)
});

function checkCorpses(match) {
    Object.keys(corpses).forEach(corpse => {
        if (new RegExp(corpse).test(match[1]))
            annotate(corpses[corpse]);
    });
}